...
public class AdditionAnswerManager : MonoBehaviour
{
    ...
    // Reference to the AdditionManager to show the correct answer as prefabs
    private AdditionManager additionManager;

    void Start()
    {
        // Find the AdditionManager in the scene
        additionManager = FindObjectOfType<AdditionManager>();
        if (additionManager == null)
        {
            Debug.LogError("AdditionManager not found in the scene!");
        }
    }
    ...
    private void CheckAnswer(int selectedAnswer, int buttonIndex)
    {
        ...
        // Show the correct answer as prefabs
        additionManager.ShowCorrectAnswer(correctAnswer);

        // Delay disabling the buttons to ensure color changes are applied first
        StartCoroutine(DisableButtonsAfterColorChange());
    ...
    private IEnumerator DisableButtonsAfterColorChange()
    {
        yield return null; // Wait one frame to allow color changes to take effect
        DisableAllButtons(); // Disable buttons after colors are updated
    }
    ...
    private IEnumerator ShowCorrectAnswerAndNextQuestion()
    {
        ...
        additionManager.PickRandomNumbers(); // Corrected method call to use AdditionManager
    }
}
